...
if ($mode == 'replace') { ?>
    <div id="ccm-files-add-asset-replace">
        ...
        <form method="post" enctype="multipart/form-data" action="/packages/legacy_updated_file_manager/tools/files/importers/single" class="ccm-file-manager-submit-single">
    ...
    <script type="text/javascript">
        ...
        $('#show_upload_files').click(function(e){
            ...
            var ufModal = $('#upload_files_modal').modal({
                "backdrop": bd,
                "remote": "<?= $uh->getToolsURL('files/import', 'legacy_updated_file_manager'); ?>?ocID=<?= $ocID; ?>&searchInstance=<?= $searchInstance; ?>"
            });
        ...
        function upl_filesUploadedDialog(searchInstance) {
            ...
            var bpModal = $('#files_uploaded_modal').modal({
                "backdrop": bd,
                "remote": "<?= $uh->getToolsURL('files/bulk_properties', 'legacy_updated_file_manager'); ?>?" + fIDstring + '&uploaded=true&searchInstance=' + searchInstance
            });
